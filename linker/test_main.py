...
def mock_search(query, k, search_type):
    mock_data = {
        "test": Dict({"score": 0.9, "metadata": {"file_path": "/path/of/test"}}),
        "linking": Dict({"score": 0.95, "metadata": {"file_path": "/path/of/linking"}}),
        "linking function": Dict(
            {"score": 0.9, "metadata": {"file_path": "/path/of/linking_function"}}
        ),
        "function test": Dict(
            {"score": 0.85, "metadata": {"file_path": "/path/of/function_test"}}
        ),
        "threshold": Dict(
            {"score": 0.75, "metadata": {"file_path": "/path/of/threshold"}}
        ),
    }
...
class TestLinkTexts(unittest.TestCase):
    ...
    def test_multiple_links(self):
        sample_text = "This is a test sentence for the linking function." + "".join(
            [" a"] * 1000
        )
        ...
        write_generator(link_texts(self.temp_dir, self.mock_vector_store))

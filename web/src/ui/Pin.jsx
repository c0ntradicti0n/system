...
function useElementById(baseId, shorterIdFinder =false, pollInterval = 200) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      let fetchedElement
      if (shorterIdFinder)
       fetchedElement = findElementById(baseId);
      else
        fetchedElement = document.getElementById(baseId)
...
export const Pin = ({ value, setHiddenId }) => {
  ...
  const labelIdRef = useElementById(
    'pin-label-' + cleanPath
  );
  const triangleIdRef = useElementById(
    'triangle-' + cleanPath,
      true
  );
...
const labelTop = labelBounds?.top;
  ...
  useEffect(() => {
    if (line)
       ...
       try {
      console.log("reposition", triangleLeft, triangleTop, labelLeft, labelTop)
        line.position();
       } catch (e) {
         console.log(e)
       }

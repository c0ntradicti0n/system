...
export function shiftIn(firstPath, secondPath, options) {
    ...
    if (options && options.left === true) {
        ...
        if (!firstPath) {
            ...
            return [secondParts.slice(0, shiftIndex+1).join('/'), secondParts.slice(shiftIndex+1).join('/')];
        }
    } else {
        // Right shift
        ...
        return [secondParts.slice(0, shiftIndex).join('/'), secondParts.slice(shiftIndex).join('/')];
    }
}

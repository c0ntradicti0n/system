...
if __name__ == "__main__":
    ...
    scheduler = ReduceLROnPlateau(
        optimizer, "min", patience=10, factor=0.9, verbose=True
    )
    ...
    for epoch in range(3000):
        ...
        indices = np.random.choice(
            len(samples), size=(batch_size, n_samples), replace=False
        )
        ...
        outputs = (
            outputs.view(batch_size, n_samples, n_samples - 1, output_dim)
            .max(dim=2)
            .values
        )
        ...
        with torch.no_grad():
            ...
            f1 = f1_score(
                predicted.view(-1), selected_labels[:, 0].view(-1), average="micro"
            )
            print(
                f"Epoch {epoch + 1}, F1 Score: {f1:.4f}, Loss: {loss.item()} {optimizer.param_groups[0]['lr']}"
            )
